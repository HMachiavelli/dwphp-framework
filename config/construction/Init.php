	...
	*/
	...
	function __construct($type = '')
	{
		...
		try {
			...
			$this->notificationErrors('Falha na função getImportDataConfiguration', $e->getMessage());
			exit;
		}
		...
		try {
			...
			$this->notificationErrors('Falha na função setSystemConfigs', $e->getMessage());
			exit;
		}
		...
		try {
			...
			$this->notificationErrors('Falha na função getApplicationUseNow', $e->getMessage());
			exit;
		}
		...
		try {
			...
			$this->notificationErrors('Falha na função getImportCurrentDataApplication', $e->getMessage());
			exit;
		}
		...
		try {
			...
			$this->notificationErrors('Falha na função setSystemConfigsCurrentApplication', $e->getMessage());
			exit;
		}
	...
	private function setSystemConfigsCurrentApplication()
	{
		...
		if ($this->getUseHttps() == 'On' || (preg_match('/^www/', $this->getAddressUri()) && $this->getUseWww() == 'Off') || (!preg_match('/^www/', $this->getAddressUri()) && $this->getUseWww() == 'On') || $k != '') {
			if (
				...
				header('Location: ' . ($this->getUseHttps() == 'On' ? 'https://' : 'http://') . $www . $this->getAddressUri());
				exit;
			}
		}
	}
		...
		if (file_exists($pathFile)) {
			...
			if (class_exists('App\\template', true)) {
					...
					if (class_exists('\App\Framework\controller', false)) {
						...
						if (isset($this->ctrlFunction) && !empty($this->ctrlFunction)) {
							try {
								...
								$this->notificationErrors('Função não encontrada:', $e->getMessage());
								exit;
							}
						}
					}
				...
				try {
					...
					$this->notificationErrors('Não inciado constructPage', $e->getMessage());
					exit;
				}
	...
	{
		$this->chaceNavegation = (bool)$chaceNavegation;
		return $this;
	}
