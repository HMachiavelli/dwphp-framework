...
$dir = PATH_ROOT . "{$s}app{$s}entity{$s}";

require_once PATH_ROOT . "{$s}vendor{$s}autoload.php";
...
if (file_exists(PATH_ROOT . "{$s}app{$s}app_config.yml")) {
	$conf_db = Yaml::parse(file_get_contents(PATH_ROOT . "{$s}app{$s}app_config.yml"));
	...
	echo "app{$s}app_config.yml não encontrado\n";
}
